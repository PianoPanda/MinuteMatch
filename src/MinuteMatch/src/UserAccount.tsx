...
const UserAccount: React.FC = () => {
  ...
  const fetchUserReviews = async () => {
    try {
      const res = await axios.get("http://localhost:3000/userreview", {
        params: { username, category: reviewCategory || undefined },
      });
      setReviewFeed(res.data);
    } catch (err) {
      console.error("Error fetching user reviews:", err);
      ...
      <div className="user-account-container">
        <div className="user-account-layout">
          ...
          <div className="panel user-details-panel">
            ...
            <h2>Groups</h2>
            <ul className="group-list">
              {groups.length > 0 ? groups.map((group, idx) => <li key={idx}>{group}</li>) : <li>No groups joined</li>}
            </ul>

            <h2>Category-Based Ratings</h2>
            <div className="category-grid">
              {reviews.length > 0 ? reviews.map(([category, score], idx) => (
                <div key={idx} className="category-box">
                  <strong>{category}</strong>
                  <div>{score}/5</div>
                </div>
          ...
          <div className="panel rate-user-panel">
            ...
            <form className="rate-form" onSubmit={handleSubmit}>
              ...
              <button type="submit" className="rate-submit">Submit Review</button>
            </form>
          </div>
          ...
          <div className="panel all-reviews">
            ...
            />
            <div className="review-feed">
              {reviewFeed.length > 0 ? reviewFeed.map((r, i) => (
                <div key={i} className="review-card">
                  <strong>{r.category}</strong> — {r.score}/5
                  <p>{r.text}</p>
                  <small>by {r.reviewer}</small>
          ...
          <div className="panel search-reviews">
            ...
            />
            <button onClick={fetchSearchUserReviews} className="rate-submit">Search Reviews</button>
            <div className="review-feed" style={{ marginTop: "1rem" }}>
              {searchUserReviews.length > 0 ? searchUserReviews.map((r, i) => (
                <div key={i} className="review-card">
                  <strong>{r.category}</strong> — {r.score}/5
                  <p>{r.text}</p>
                  <small>by {r.reviewer}</small>
